...
This command delegates content generation to the **wiz-planner** agent (`wiz-planner`), which provides verbose, research-focused planning output suitable for strategic phase decomposition.
...
The `wiz-planner` agent provides verbose, research-focused planning output suitable for strategic phase decomposition. When you reference `wiz-planner` in the next step, it will automatically provide comprehensive, detailed phase planning content.
...
Reference the `wiz-planner` agent with the beginning of the following prompt:
