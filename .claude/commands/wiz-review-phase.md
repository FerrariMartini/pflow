...
This command delegates detailed review to the **wiz-reviewer** agent (`wiz-reviewer`), which provides comprehensive phase review output suitable for quality assurance activities.
...
**Workflow:**
1. Reference `wiz-reviewer` with the prompt template below
...
Reference the `wiz-reviewer` agent with the following prompt:
...
- Review delegates to wiz-reviewer agent (`wiz-reviewer`)
