...
This command delegates content generation to the **wiz-planner** agent (`wiz-planner`), which provides verbose, research-focused planning output suitable for strategic planning activities.
...
The `wiz-planner` agent provides verbose, research-focused planning output suitable for strategic planning activities. When you reference `wiz-planner` in the next step, it will automatically provide comprehensive, detailed planning content.
...
Reference the `wiz-planner` agent with the following prompt (including the codebase analysis):
...
- Command delegates to `wiz-planner` agent (reference `wiz-planner`)
