...
3. `/wiz-phases <slug>` decompõe o PRD em **fases**, e `/wiz-milestones <slug>` decompõe cada fase em **milestones** identificadas no formato `P0XM0Y` (fase X, milestone Y). O estado de planejamento fica em `.wiz/<slug>/` — `prd.md`, `phases/phaseN.md`, e depois `reviews/` e `retros/`.
